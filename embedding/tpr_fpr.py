    ...
    for target in targets:
        ...
        for time in gt_target_times:
            ...
            if not potential_match:
                #false negative
                # TODO(mmaz): we should not be producing a confidence value for FNs
        ...
        for time in found_target_times:
            ...
            # TODO(mmaz): garbage confidence values
            if potential_match: #true positive
                detections.append([target, time, 1, 'tp'])
            else: #false positive
                detections.append([target, time, 1, 'fp'])
